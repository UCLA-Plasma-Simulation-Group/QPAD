...
def install_hdf5():
    ...
    sleep(1.0)
    hdfpath = installpath + "/hdf5-1.13.0"
    if not os.path.exists(hdfpath):
        print(f"Downloading hdf5 repository to {hdfpath} ...", end = '', flush = True)
        if not os.path.exists(installpath + "/hdf5-1.13.0.tar.gz"):
            subprocess.run(["wget", "https://support.hdfgroup.org/ftp/HDF5/releases/hdf5-1.13/hdf5-1.13.0/src/hdf5-1.13.0.tar.gz"])
        subprocess.run(["tar", "-xvzf", "hdf5-1.13.0.tar.gz"], stdout=logfile)
    else:
...
def install_json_fortran():
    ...
    if f"export DYLD_LIBRARY_PATH={buildpath}/lib:$DYLD_LIBRARY_PATH" not in bashcontents:
        with open(bashpath, "a+") as bashfile:
            bashfile.write(f"export DYLD_LIBRARY_PATH={buildpath}:$DYLD_LIBRARY_PATH \n")
        bashfile.close()
